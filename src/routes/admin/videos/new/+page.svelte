...
<svelte:head>
	<title>Create new video | Full Stack Jack</title>
</svelte:head>
	...
	<div class="flex items-center justify-center px-4 sm:px-6 lg:px-8">
		<div class="max-w-md w-full">
			...
			<form class="mt-8 space-y-6" method="POST" use:enhance>
				...
				<div class="rounded-md shadow-sm -space-y-px mb-1">
					<div>
						<label for="host-id" class="sr-only">Video id</label>
						<input
							bind:value={host_id}
							id="host-id"
							name="host-id"
				...
				<div class="rounded-md shadow-sm -space-y-px mb-1">
					<div>
						...
						<select bind:value={topicId} id="topic-id" name="topic-id">
							{#each data.topics as topic}
								<option value={topic.id}>{topic.displayName || topic.name}</option>
							{/each}
						</select>
					</div>
				</div>
