<script lang="ts">
	...
	function handleSubmitSubscription() {
		return async ({ result }: { result: ActionResult }) => {
			if (result.type === 'success' && result.data?.stripeUrl) {
...
<svelte:head>
	<title>Pro Membership | Full Stack Jack</title>
</svelte:head>
		...
		>
			<div class="relative -mx-4 group md:w-6/12 md:mx-0 lg:w-5/12 z-20">
				...
				<div class="relative p-6 space-y-6 lg:p-8">
					...
					<ul class="w-max space-y-4 py-6 m-auto text-gray-600">
						<li class="space-x-2">
					...
					{#if isLoggedIn}
						<form method="POST" use:enhance={handleSubmitSubscription}>
							<!-- <input type="hidden" name="lookupKey" value="price_1MMXRwGd1agtszYKtyDkfkdv" /> -->
			...
			<div class="relative group md:w-6/12 lg:w-7/12 z-10">
				...
				<div class="relative p-6 pt-16 md:p-8 md:pl-12  md:rounded-r-2xl lg:pl-20 lg:p-16">
					<ul class="space-y-4 py-6 text-gray-600">
						<li class="space-x-2">
							<span class="text-purple-500 font-semibold">&check;</span>
					...
					<div class="mt-6 flex justify-between gap-6">
						...
						<img class="w-8 lg:w-16" src="/images/laravel.svg" loading="lazy" alt="bissell" />
						<img class="w-6 lg:w-12" src="/images/linux.svg" loading="lazy" alt="ge" />
						<img class="w-20 lg:w-28" src="/images/docker.svg" loading="lazy" alt="microsoft" />
					</div>
				</div>
			</div>
		</div>
